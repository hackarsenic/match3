...
DrawBoard::DrawBoard(unsigned render_window_w, unsigned render_window_h)
...
{
    d_scoreFont.loadFromFile((GameConfig::GetInstance().GetResourcePath() / "arial.ttf").string());
...
void DrawBoard::SetPosition(float x, float y)
{
    d_positionX = x;
    d_positionY = y;
}
...
constexpr void DrawBoard::InitBoardDimensions()
{
    ...
    for (unsigned i = 0; i < d_rows * d_columns; ++i) {
        const unsigned x = i % d_columns;
        const unsigned y = i / d_columns;
...
std::shared_ptr<Gem> DrawBoard::GetGem(unsigned column, unsigned row)
{
...
void DrawBoard::Add(unsigned column, unsigned row, Gem::Color color)
{
    if (row >= 0 && row < d_rows && column >= 0 && column < d_columns) {
        ...
        const unsigned gem_index = row * d_columns + column;

        const unsigned x = gem_index % d_columns;
        const unsigned y = gem_index / d_columns;
        const float cur_gem_scale_factor = d_targetGemSize / std::max(gem->GetSprite()->getTexture()->getSize().x, gem->GetSprite()->getTexture()->getSize().y);
        gem->GetSprite()->setPosition(d_positionX + x * d_cellWidth + d_gemPositioningTweakFactor * cur_gem_scale_factor, d_positionY + y * d_cellHeight + d_gemPositioningTweakFactor * cur_gem_scale_factor);
        gem->GetSprite()->setScale(cur_gem_scale_factor, cur_gem_scale_factor);
...
void DrawBoard::MarkRemoved(unsigned column, unsigned row)
{
    if (row >= 0 && row < d_rows && column >= 0 && column < d_columns) {
...
void DrawBoard::Move(const std::shared_ptr<Gem> &move_gem, unsigned column, unsigned row)
{
    ...
    if (row >= 0 && row < d_rows && column >= 0 && column < d_columns) {
        ...
        const unsigned move_x = d_positionX + x * d_cellWidth + d_gemPositioningTweakFactor * cur_gem_scale_factor;
        const unsigned move_y = d_positionY + y * d_cellHeight + d_gemPositioningTweakFactor * cur_gem_scale_factor;
...
void DrawBoard::GemsSelectedHandler(std::function<void(unsigned, unsigned, unsigned, unsigned)> handlerFunction)
{
    d_GemsSelected = std::move(handlerFunction);
}

void DrawBoard::BombSelectedHandler(std::function<bool(unsigned, unsigned)> handlerFunction)
{
...
void DrawBoard::OnMousePress(sf::Event *event)
{
    ...
    if (event->type == event->MouseButtonPressed && event->mouseButton.button == sf::Mouse::Left && !d_isGemSelected && !d_isAnimating) {
        ...
        const unsigned x_cell = (event->mouseButton.x - d_positionX) / d_cellWidth;
        const unsigned y_cell = (event->mouseButton.y - d_positionY) / d_cellHeight;
        ...
        if (d_state == DrawBoardState::SelectFirstGemState || d_state == DrawBoardState::RemoveGemsState) {
            ...
            sf::Vector2f scale_to(current_sprite->getScale().x * 0.75f, current_sprite->getScale().x * 0.75f);
    ...
    if (event->type == event->MouseButtonPressed && event->mouseButton.button == sf::Mouse::Left && d_isGemSelected) {
        ...
        unsigned x_cell = (event->mouseButton.x - d_positionX) / d_cellWidth;
        unsigned y_cell = (event->mouseButton.y - d_positionY) / d_cellHeight;
        ...
        const unsigned second_cell = y_cell * d_columns + x_cell;
...
bool DrawBoard::DetonateBomb(unsigned cell)
{
...
void DrawBoard::GenerateObjectives()
{
    ...
    for (const auto [key, value] : GameConfig::GetInstance().GetObjectives()) {
        ...
        objective_text.setCharacterSize(47);
...
{
    const sf::Texture &center_texture = ResourceManager::GetTexture((GameConfig::GetInstance().GetResourcePath() / "tile_2.png").string());
    ...
    const sf::Texture& borders_texture = ResourceManager::GetTexture((GameConfig::GetInstance().GetResourcePath() / "tile_1.png").string());
...
{
    const sf::Texture& tile1_texture = ResourceManager::GetTexture((GameConfig::GetInstance().GetResourcePath() / "tile_1.png").string());
    ...
    const sf::Texture& tile2_texture = ResourceManager::GetTexture((GameConfig::GetInstance().GetResourcePath() / "tile_2.png").string());
